...
class SequentialPriceTask(GameBasedTask):
    ...
    def get_task_score(self, prompt_data):
        ...
        if p2_bid < prompt_data.info.p1_bid: # Using prompt_data instead of self.game to ensure consistency
            score = p2_bid - prompt_data.info.p2_true_cost if prompt_data.info.profit_maximisation else 1.0
        else:
            score = 0.0
        return score
...
class SequentialPriceGame:
    ...
    def generate_episode_data(self):
        ...
        self.efficiency_factor = round(torch.FloatTensor(1).uniform_(0.7, 1.3).item(), 2)
        ...
        self.annual_volume = round(total_equipment_cost / (target_per_unit * self.machine_life_years), 2)
...
if __name__ == '__main__':
    ...
    sequential_price_task = SequentialPriceTask(language_aspect=sequential_price_language_aspect)
