...
def single_query(
    ...
    print(f'Requesting result from {model}')
    start = time.time()
    full_response_data = client.beta.chat.completions.parse(
        model=model,
        messages=messages,
    )
    response_text = full_response_data.choices[0].message.content
    request_time = time.time() - start
    print('Request returned in {:.1f}s'.format(request_time))
    return response_text, full_response_data
